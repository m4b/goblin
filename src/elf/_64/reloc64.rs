...
elf_rela_impure_impl!(
    ...
    pub fn parse<S: scroll::Scroll<usize>>(fd: &S, offset: usize, size: usize, little_endian: bool, is_rela: bool) -> io::Result<Vec<Reloc>> {
        ...
        for _ in 0..count {
            ...
            reloc.r_offset = try!(fd.read_u64(&mut offset, little_endian)) as usize;
            let info = try!(fd.read_u64(&mut offset, little_endian));
            reloc.r_info = info as usize;
            if is_rela { reloc.r_addend = try!(fd.read_i64(&mut offset, little_endian)) as isize; }
