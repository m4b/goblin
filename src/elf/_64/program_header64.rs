    ...
    impl ProgramHeader {
        ...
        pub fn parse<S: scroll::Scroll>(fd: &S, offset: u64, count: usize, little_endian: bool) -> io::Result<Vec<ProgramHeader>> {
            ...
            for _ in 0..count {
                ...
                phdr.p_offset = try!(fd.read_u64(&mut offset, little_endian));
                phdr.p_vaddr = try!(fd.read_u64(&mut offset, little_endian));
                phdr.p_paddr = try!(fd.read_u64(&mut offset, little_endian));
                phdr.p_filesz = try!(fd.read_u64(&mut offset, little_endian));
                phdr.p_memsz = try!(fd.read_u64(&mut offset, little_endian));
                phdr.p_align = try!(fd.read_u64(&mut offset, little_endian));
                phdrs.push(phdr);
            }
            Ok(phdrs)
        }
