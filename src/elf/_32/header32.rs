...
elf_header_impure_impl!(
    impl Header {
        ...
        pub fn parse<S: scroll::Scroll>(buffer: &S) -> io::Result<Header> {
            ...
            elf_header.e_entry = try!(buffer.read_u32(&mut offset, little_endian));
            elf_header.e_phoff = try!(buffer.read_u32(&mut offset, little_endian));
            elf_header.e_shoff = try!(buffer.read_u32(&mut offset, little_endian));
